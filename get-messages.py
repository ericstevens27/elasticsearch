...
def createquery(qid):
    ...
    gte = q["query"]["bool"]["must"][1]["range"]['@timestamp']['gte']
    lte = q["query"]["bool"]["must"][1]["range"]['@timestamp']['lte']
    if options.debug:
        print("Check query data: Date from is {}, Date to is {}".format(gte, lte))
    return q
...
output_file_base = "_messages_aug.csv"
...
if options.test:
    json_files = ["test_devices.json", "test2_devices.json"]
    print("[WARNING]: Running in Test Mode")
else:
    json_files = ["normal_devices.json", "suspicious_devices.json"]
